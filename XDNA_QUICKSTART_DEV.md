...
| `GGML_XDNA_NUM_COLS` | **4** | Сколько колонок NPU запрашивать. Наш харнесс ставит 8 (XDNA 2). `compile.py` проверяет, что у устройства их не меньше, и отказывается компилировать, если запрошено больше, чем есть |
...
| XDNA 1 не проверялся | Детекция колонок работает (Phoenix/Hawk Point отдадут 4, и запрос 8 будет отклонён с внятной ошибкой), но самого железа у нас нет — ни один путь на нём не прогонялся | нужен доброволец с Ryzen 7040/8040 |
| Prefill медленнее CPU | ~100 vs ~200 т/с на 1B Q4_0 | выигрыш только в decode |
| Разовые ~430 мс на 1-м токене | Упаковка+заливка весовых BO всех 16 слоёв; видно как `w=434282 us` в `XDNA_F3BEST_GLUETIME` на первом decode-токене. Наблюдаемое `мс/ток ≈ 28 + 430/n` | мерить при `-n 192`+; иначе decode занижен (29.1 при n=64 против 33.2 при n=192) |
