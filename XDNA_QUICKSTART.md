...
| NPU (f3best LOOP) | 29.1 т/с | **36.8 т/с** | ~100 т/с |
| выигрыш | 2.7× | **3.5×** | 0.5× |

Установившаяся стоимость токена — около 27 мс; из них ~18 мс это 16 диспатчей слоёв,
~4 мс — vocab-проекция на CPU, остальное — обход графа и делегаты.

### Ещё +7% за одну переквантацию

Vocab-проекция считается на CPU, и её стоимость зависит от типа `token_embd`. По умолчанию
в Q4_0-сборках он остаётся **Q6_K**. Переквантовав его в Q4_0, получаем **39.3 т/с вместо
36.8** — при том же коде:

```powershell
.\build\bin\Release\llama-quantize.exe --token-embedding-type q4_0 `
    models\llama-3.2-1b-instruct-BF16.gguf models\llama-3.2-1b-q4_0-vocabq4.gguf q4_0
```

Цена — качество: на wiki.test перплексия растёт с 18.89 до 19.30, то есть +2.2%, что
укладывается в доверительный интервал самих замеров.
...
### Performance build (AVX2 host)

Базовая сборка целится в максимальную совместимость — она компилируется **без SIMD** (`GGML_NATIVE=ON` → AVX2/AVX/FMA выключены). На современном x86_64 это искусственно ограничивает CPU-путь (lm_head, reduce) примерно до 11 t/s.

Если машина-цель поддерживает AVX2 (любой x86_64 с 2013 года), используйте `+avx2`-пресет — это однострочная замена, которая включает runtime-dispatch на AVX2 и даёт ~75 t/s на чистом CPU и ~78 t/s с NPU:

```powershell
cmake --preset x64-windows-msvc-release+avx2
cmake --build build-x64-windows-msvc-release+avx2 --config Release
```

Есть также `+avx512` (для процессоров с AVX-512 VNNI — измеренный прирост над AVX2 ≈ 0%, чистый AVX2 уже даёт весь эффект).

> ⚠️ **Бинарник с AVX2 упадёт (illegal instruction) на старых x86_64 без AVX2.** `/arch:AVX2` в MSVC жёстко кодирует AVX2 во всём TU — рантайм-фоллбэка нет. Пресет предназначен **только на свою машину**, не для пересылки другим. Для публичного дистрибутива используйте базовый пресет (без SIMD) — он запустится везде, просто медленнее.
