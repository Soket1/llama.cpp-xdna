...
OUT=Path(r'C:/llama.cpp-xdna/.historical-artifact-sweep/bench_qptb_vs_rr');OUT.mkdir(parents=True,exist_ok=True)
QP=Path(r'C:/llama.cpp-xdna/.historical-artifact-sweep/qp-tb-source'); CUR=Path(r'C:/llama.cpp-xdna')
configs={
 'qp_tb':{'root':QP,'exe':QP/'build/bin/Release/llama-cli.exe','cache':QP/'npu_kernels_win_8col','key':'decode_layer_f3best_K2048_H8192_sl256_d64_ag4_kv8_g32_mc_preq_vexp_vreg_dq8_qp_tb','toggles':{'F3BEST_TRIPLE_B':'1'}},
...
man={'model':str(MODEL),'prompt':PROMPT,'n_predict':N,'configs':{}}
for n,c in configs.items():
 x=c['cache']/(c['key']+'.xclbin');i=c['cache']/(c['key']+'.insts');man['configs'][n]={'root':str(c['root']),'exe':str(c['exe']),'key':c['key'],'toggles':c['toggles'],'xclbin_sha256':hashlib.sha256(x.read_bytes()).hexdigest(),'xclbin_bytes':x.stat().st_size,'insts_sha256':hashlib.sha256(i.read_bytes()).hexdigest()}
(OUT/'manifest.json').write_text(json.dumps(man,ensure_ascii=False,indent=2),encoding='utf-8')
res={n:[]for n in configs}
for ix,n in enumerate(['qp_tb','current_rr']*3,1):
 c=configs[n];print(f'=== {ix}/6 {n} ===',flush=True)
 r=subprocess.run(args(c),cwd=c['root'],env=environ(c),capture_output=True,text=True,errors='replace',timeout=2000)
 p=OUT/f'{ix:02d}_{n}';p.with_suffix('.stdout.txt').write_text(r.stdout,encoding='utf-8');p.with_suffix('.stderr.txt').write_text(r.stderr,encoding='utf-8')
 ...
 if c['key'] not in ks:raise RuntimeError(f'{n} run{ix}: wrong key {ks[-4:]}')
 rate=float(ps[-1][1]);res[n].append(rate);print(f'{n} {rate:.2f} t/s key={ks[-1]}',flush=True)
sumry={n:{'rates':v,'median':statistics.median(v),'mean':statistics.fmean(v),'min':min(v),'max':max(v)}for n,v in res.items()};sumry['delta_qptb_minus_current_rr']=sumry['qp_tb']['median']-sumry['current_rr']['median'];(OUT/'summary.json').write_text(json.dumps(sumry,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(sumry,ensure_ascii=False),flush=True)
